-- Migración 21 (2026-08-29): corrige ambigüedad PL/pgSQL en fn_registrar_comprobante (fix de migración 20)
-- En RETURNS TABLE(id_comprobante uuid, id_pedido uuid) esos nombres son variables de salida;
-- el WHERE del UPDATE debe calificar la columna con el alias de la tabla (error "column reference id_comprobante is ambiguous").
-- Mantiene la idempotencia de reenvío de la migración 20 (D5, Reglas 2/7):
