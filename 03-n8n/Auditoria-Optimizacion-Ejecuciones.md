# Auditoría de optimización de ejecuciones n8n — RSUELVO v2

**Versión vigente:** v2 · 2026-09-11. **Cuenta:** `rsuelvotest.app.n8n.cloud`.
**Alcance:** 16 workflows canónicos de Matriz §0; WF-30 archivado excluido.
**Solo lectura:** en v2 se leyeron SQL/documentación local y se verificaron puntualmente WF-25-B, WF-10 y WF-13 publicados. No se consultó ni modificó la BD, ni se ejecutaron/modificaron workflows. El único archivo actualizado es este informe. Los demás grafos y la muestra de ejecuciones conservan el corte de v1 (2026-09-11, aproximadamente 09:53 America/La_Paz).

## Cambios v2

1. **P1 invalidado y reescrito:** m38 reemplaza m37. Destino = una ciudad/población en texto libre; zona eliminada. La pregunta live actual es correcta. Se retira el supuesto ahorro de 5 T/1 Q por pedir CIUDAD+ZONA. No se prescribe cambiar la pregunta.
2. **m40 incorporada:** resolución por `(p_id_comercio, p_sku, p_id_sucursal)`, `fn_variante_efectiva`, listado por sucursal y snapshot efectivo de pedido. WF-10/WF-13 ya adaptados, sin ahorro incremental por esa adaptación. Reutilización y lotes deben conservar sucursal y correspondencia por ítem.
3. **Firmas resueltas documentalmente:** archivos 31/34/35/36/40 y las definiciones finales aplicables de `06_functions.sql`, inventariadas en §2.1. No quedan como solicitudes al orquestador. Leer SQL local no equivale a ejecutarlo ni a certificar el despliegue de la BD.
4. **Cinco bugs excluidos del ahorro:** F1–F5 de [Intervención de fixes funcionales](PROMPT-INTERVENCION-FIXES-FUNCIONALES.md), referentes a WF-24, WF-20 y WF-12, se remiten a ese paquete. No se releyeron esos tres workflows en v2 para re-auditarlos ni se les asigna ahorro.
5. **Prioridades y escenarios corregidos:** P1 pasa a verificación cerrada con 0 T/0 Q; el top 5 activo pasa a P2/P3/P4/P5/P7. Se conserva la distinción técnica T/cuota Q; los mensajes de transporte ahora se cuentan como nombre, punto y ciudad, sin zona.
6. **Historial preservado:** v1 y su addendum permanecen íntegros al final, bajo “Historial v1 — no vigente”. Sus propuestas invalidadas no rigen esta revisión.

Fuentes de corrección: [OBS-005 vigente](../07-Control-de-Calidad/Observaciones-Usuario.md#obs-005-ciudad-de-destino-en-un-solo-mensaje-del-comprador-m38-solo-ciudad-zona-eliminada), [bitácora 2026-09-09/10/11](../00-Index/ESTADO-EJECUCION.md), [encargo v2](PROMPT-CODEX-OPTIMIZACION-EJECUCIONES-V2.md).
...
### Top 5 oportunidades activas por ahorro/riesgo

| Orden / propuesta | Oportunidad | Ahorro relativo estimado | Evidencia / riesgo |
|---|---|---|---|
| 1 / P2 | Deduplicación efectiva de WF-25-C y repetición de WF-25-A | Por callback repetido frenado antes de 80: **1 T, 0 Q directo**, más status evitados. Si se evita también el callback en el emisor: **2 T + 1 Q** | Muestra v1 prueba bypass, no doble envío por ejecución. Medio; recuperación tras fallo parcial |
| 2 / P3 | Suprimir en origen entradas técnicas realmente prescindibles de WF-02 | **1 T + 1 Q** por POST evitado antes de n8n; filtro interno **0 Q** | Status ya termina temprano. Medio/alto; preservar eventos necesarios y payloads mixtos |
| 3 / P4 | Evitar despertares vacíos/repetidos de WF-13 | **1 T + 1 Q** por vacío evitado; **K−1 T/Q** si K callbacks equivalentes pasan a uno | Volumen externo desconocido. Medio/alto; aislamiento por sucursal e ítem m40 como condición previa |
| 4 / P5 | Evitar reenvíos inducidos por errores de extracción/contexto en WF-21/22/23 | Por reenvío realmente eliminado con pedido y ACK manual: aproximadamente **6 T + 1 Q**, más status | Hipótesis de ahorro, no frecuencia medida; excluye íntegramente F1–F5. Medio/alto |
| 5 / P7 | Inline de la transformación de WF-03 | **1 T por mensaje nuevo admitido, 0 Q** | Transformación pequeña; beneficio técnico, no ahorro de cuota. Bajo/medio según otros callers |

**P1: 0 T/0 Q adicionales**; la simplificación m38 ya existe. P6 (omitir OCR opcional) y P8 (contexto/payloads) siguen siendo opciones condicionadas, sin ahorro directo de cuota. No sumar oportunidades solapadas ni inferir porcentaje mensual sin Usage. Las correcciones F1–F5 tienen **ahorro excluido**, no son oportunidades de esta tabla.
...
Precedencia: reglas/decisiones del PROMPT para lo normativo; Matriz §0 para IDs; JSON publicado live para lo que está implementado. Las contradicciones se registran aquí sin editar fuentes. La autorización de actualizar solo este informe prevalece sobre la regla documental general de actualizar matrices.

**Corte v1: 16/16 lecturas live satisfactorias:** todos activos, no archivados, `activeVersion.sameAsDraft=true`; versión publicada coincide con el borrador consultado. No se usó el nombre del nodo como prueba de su destino.
...
**Reverificación v2:** WF-25-B (`5bcc1c92`), WF-10 (`9484d2eb`) y WF-13 (`27b59091`) conservan las versiones publicadas del inventario y `sameAsDraft=true`. No se renovó la muestra de ejecuciones.

Los 17 JSONs del directorio principal representan 16 flujos y una segunda copia de WF-25-C; `_BORRAR` se identificó como histórico y no integra los 16. Los exports tienen referencias de la cuenta anterior y varios carecen de id/version internos. Diferencias materiales: WF-22 export tiene 6 nodos frente a 10 live; WF-25-B 31 frente a 32; la diferencia de destino de WF-24 pertenece a F1 del paquete separado, sin reauditoría v2. No importar estos exports como corrección.

### 2.1 Contratos resueltos en SQL local: m31/m34–38/m35/m36/m40

Fuentes: [31](../02-Base-de-Datos/sql/31_fn_rechazar_lista_espera.sql), [34 final, incluye m38](../02-Base-de-Datos/sql/34_entrega_captura_destino.sql), [35](../02-Base-de-Datos/sql/35_lista_pendiente_momento1.sql), [36](../02-Base-de-Datos/sql/36_guia_foto.sql), [40](../02-Base-de-Datos/sql/40_variante_sucursal_overrides.sql), [06_functions.sql](../02-Base-de-Datos/sql/06_functions.sql). Las firmas siguientes son las definiciones documentadas, no inferencias del nombre de un nodo. En 06 existen redefiniciones: para pedido y notificación se tomó la última definición m40, no el cuerpo anterior con precio global.

| Función / firma | Retorno y comportamiento relevante | Fuente / trazabilidad |
|---|---|---|
| `fn_rechazar_lista_espera(p_id_lista_espera uuid)` | jsonb: OPORTUNIDAD_RECHAZADA, estados anterior/nuevo, id_sucursal/id_variante; NOTIFICADO→RECHAZADO, ESPERANDO→CANCELADO; bloquea fila | 31 + 06; WF-14, HU-046–049; tbl_lista_espera |
| `fn_pendiente_lista(p_id_comercio uuid, p_id_sucursal uuid, p_id_variante uuid, p_id_cliente uuid)` | jsonb PENDIENTE_LISTA; upsert del pendiente por cliente | 35 + 06; WF-10, HU-043,044; tbl_lista_pendiente |
| `fn_aceptar_pendiente_lista(p_telefono text)` | jsonb SIN_PENDIENTE / YA_EN_LISTA / PENDIENTE_OK; consume pendiente y devuelve contexto. **No inserta en tbl_lista_espera:** la inscripción sigue por WF-12 | 35 + 06; WF-14/12, HU-043,044 |
| `fn_rechazar_pendiente_lista(p_telefono text)` | jsonb SIN_PENDIENTE / LISTA_RECHAZADA; elimina pendiente | 35 + 06; WF-14, HU-043,044 |
| `fn_iniciar_captura_destino(p_id_pedido uuid, p_id_punto_entrega uuid)` | jsonb CAPTURA_INICIADA, pedido/punto/transportadora; exige PAGADO, sin envío, punto transporte activo de la sucursal | 34 + 06; WF-25-B, HU-077,086; tbl_entrega_captura |
| `fn_entrega_captura_estado(p_telefono text)` | jsonb captura false o true, paso DESTINO y contexto; restringe al pedido PAGADO sin envío más reciente del cliente | 34 + 06; WF-25-B, HU-077,086 |
| `fn_procesar_captura_destino(p_telefono text, p_texto text)` | jsonb SIN_CAPTURA / OPCION_INVALIDA / CAPTURA_REINICIADA o retorno de fn_registrar_entrega. Texto vacío: excepción. Dígitos: re-selección. Otro texto: trim y recorte a 120 caracteres como ciudad; destino_zona=NULL; registra y elimina captura | 34 final m38 + 06; WF-25-B, HU-077,086; tbl_entrega_captura/tbl_envios |
| `fn_pedido_entrega_pendiente(p_telefono text)` | jsonb de contexto de entrega pendiente; contrato de entrada compartido por router y captura | 06; WF-04/25-B, HU-123,077,086; tbl_pedidos/tbl_clientes |
| `fn_listar_puntos_entrega(p_id_sucursal uuid)` | jsonb del catálogo de puntos por sucursal; no catálogo global | 06; WF-25-B, HU-077,086; tbl_puntos_entrega |
| `fn_registrar_entrega(p_id_pedido uuid, p_datos jsonb)` | jsonb ENTREGA_REGISTRADA; llamada final de captura. Para transporte m38, la captura le entrega destino_ciudad y no exige zona al comprador | Firma en 06; llamada final en 34 y decisión m38 en bitácora; WF-25-B, HU-086; tbl_envios |
| `fn_registrar_guia(p_id_envio uuid, p_numero_guia text DEFAULT NULL, p_guia_foto_url text DEFAULT NULL)` | jsonb GUIA_REGISTRADA, id_envio, número/foto y tipo_punto. Requiere al menos número o foto; ENVIO_TRANSPORTE/PUNTO_LOCAL y PREPARANDO/ASIGNADO/EN_RUTA. Actualiza y hace pg_net a entrega/estado con motivo guia_registrada | 36 + 06; WF-25-C, HU-092–094; tbl_envios, Storage guias-envios |
| `fn_variante_efectiva(p_id_variante uuid, p_id_sucursal uuid)` | TABLE(nombre text, precio numeric, activo boolean); COALESCE de override nullable por sucursal con valor global | 40 + 06; WF-10/13 y cadena de pedido, HU-037–045,056; tbl_variantes/tbl_variante_sucursal |
| `fn_resolver_variante_por_sku(p_id_comercio uuid, p_sku text, p_id_sucursal uuid DEFAULT NULL)` | TABLE(id_variante uuid, nombre text, precio numeric, id_producto uuid); SKU de 6 caracteres, comercio y activo efectivo. La sobrecarga anterior `(uuid,text)` se elimina | 40 + 06; WF-10, HU-037–041 |
| `fn_listar_variantes_sucursal(p_id_sucursal uuid)` | jsonb con variantes: valores globales/efectivos, tiene_override, stock_actual/stock_reservado; catálogo del comercio, stocks ausentes coalescidos a 0 | 40 + 06; catálogo/app; contexto de WF-10/13, HU-037–045 |
| `fn_crear_pedido_desde_reserva(p_id_reserva uuid)` | uuid de pedido; reentrada por id_pedido existente; usa nombre/precio efectivos de la sucursal de reserva y congela snapshot/precio_unitario | 06, redefinición m40; WF-20, HU-042,056; tbl_pedidos/tbl_pedido_detalles |
| `fn_notificar_siguiente_lista_espera(p_id_sucursal uuid, p_id_variante uuid)` | jsonb LISTA_VACIA / CLIENTE_NOTIFICADO; ids, fecha_expiracion, nombre/precio efectivos, tiempo_aceptacion_minutos; selección atómica con SKIP LOCKED | 06, definición final m40; WF-13/14, HU-045–049; tbl_lista_espera |
| `fn_es_admin_o_cajero_comercio(p_id_comercio uuid)` | boolean; helper de permisos sobre catálogo global del comercio | 40 + 06; soporte catálogo, contexto WF-10/13; tbl_usuario_comercio |
| `fn_puede_gestionar_catalogo(p_id_comercio uuid, p_id_sucursal uuid)` | boolean; ADMIN o CAJERO de esa sucursal, además de accesos backend/superadmin definidos en SQL | 40 + 06; soporte catálogo, contexto WF-10/13; tbl_variante_sucursal |

**Consecuencias m40:** el parámetro de sucursal tiene DEFAULT NULL en SQL, pero la orquestación comercial debe enviarlo explícitamente. No volver al resolver de dos argumentos ni calcular COALESCE de precios en Code. La variante conserva su SKU; nombre/precio/activo efectivos dependen de sucursal. El pedido ya creado conserva su snapshot: no recalcular ventas pasadas con catálogo actual.

**Comprobación live v2:** WF-10 Lookup SKU envía los tres parámetros. WF-13 Notify Next extrae nombre/precio/tiempo de la RPC; Lookup Client & Product consulta teléfono/SKU/provider, no sustituye esos valores por precio global. La adaptación ya realizada representa **0 T/0 Q de ahorro incremental**.

**Límite de reutilización:** fn_aceptar_pendiente_lista (35) devuelve nombre/precio leídos de tbl_variantes global; esos campos no son una fuente m40 para precio efectivo. Conservar sus identificadores y obtener valores de la RPC efectiva correspondiente cuando deban mostrarse. No convertir el retorno de Momento 1 en caché de precio para 10/13/20. Esto es una restricción de optimización, no otro bug cuantificado.

### Muestra de ejecuciones existentes (corte v1)
...
**NO Momento 2:** el nodo “Notificar Siguiente” es una RPC, no un Execute WF-13. La Matriz describe la notificación al siguiente, pero el grafo live de 14 no contiene llamada a WF-13 ni construcción del aviso al siguiente destinatario. La definición final de fn_notificar_siguiente_lista_espera en 06 no hace pg_net: selecciona, marca y devuelve el resultado. No contar WF-13 como hijo de esa RPC. La eventual notificación por cron/trigger externo depende de la topología desplegada (§9); no se certifica su recepción.

### 3.2 Pago automático: conteo de diseño, sin reauditoría F1–F5

Tras comprobante legible y VERIFICACION_INICIADA: B→21→22 = 5 T; ACK 21→80 = +1 T; 21→23→24→salida = +3 T. Si PAGADO dispara además 25-A→80: +2 T y +1 Q. El diseño suma **11 T y 2 Q** antes de status.

No es una venta automática live certificada: el paquete separado F1–F3 define la reparación y coordinación del aviso. Esta v2 **no cuantifica ahorro de ese paquete ni presupone su resultado final**. También conserva del corte v1 los límites de contexto 21→23 (Build WF-80 Ack descarta datos de verificación) y payloads de rechazo/manual de 23. Son los únicos aspectos de ese recorrido tratados en P5.

Rechazo/manual automático bien encaminado: B+21+22+ACK80+23+respuesta80 = **8 T, 1 Q**; puede interrumpirse antes por esos límites. El conteo efectivo tras intervención se medirá por separado.
...
Para transporte, añadir un mensaje de ciudad/población en texto libre (m38): **+5 T, +1 Q**. Cada evento adicional de preparación/asignación/ruta/guía añade **+2 T, +1 Q** bajo la ruta notificable actual; no asumir que todos ocurren. Con **m** mensajes de datos de entrega y **c** callbacks notificables de estado/guía, el total manual es:

**T = 14 + 5m + 2c + S; Q = 3 + m + c + S.**

m=2 para nombre y punto local; m=3 si transporte agrega ciudad. La pregunta actual ya admite esa única respuesta, sin formato CIUDAD:/ZONA:. No se atribuye a P1 una corrección adicional ni ahorro. Una corrección o re-selección realmente ocurrida añade otra unidad a m, pero su frecuencia no fue medida. Para venta desde lista, sustituir el SKU inicial por las etapas efectivamente vividas: SIN_STOCK 5/1 + SI inicial 6/1 + aviso 13→80 2/1 (solo si hubo un destinatario) + SI oportunidad 6/1. No atribuir a una única venta todas las notificaciones de un lote.
...
| 10 / HU-037–041 | 1 por SKU; 20 solo reserva nueva/existente | Mantener resolver de 3 parámetros y valores efectivos m40 ya usados | 0 adicional demostrado | Medio por sucursal/stock/concurrencia |
| 12 / HU-043,044 | 1 por SI Momento 1 aceptable | Mantener consentimiento; F5 en intervención separada | Ahorro excluido para F5 | Medio |
| 13 / HU-045,049,053 | 1 por callback; N hijos 80 | Despertares útiles por grupo/sucursal, con correlación m40 por ítem | K−1 raíces si K→1; 1 raíz por vacío suprimido | Medio/alto; cron/turnos en BD |
...
| 20 / HU-042,056 | 1 por reserva nueva/existente aceptada | Mantener reutilización por 10 y 14 y snapshot efectivo m40; F4 separado | 0 probado | Alto si se omite cobro/reenvío legítimo |
...
| 22 / HU-059,146 | 1 por comprobante con pedido en ruta prevista | Mantener extractor; error técnico explícito (P5); F1 excluido | Reenvíos evitados condicionados; 0 Q directo | Medio/alto |
...
| 24 / HU-076,078–081 | 1 al confirmar automático, sin E2E certificado | F1–F3 en paquete separado; sin propuesta nueva de ahorro | Ahorro excluido | Intervención funcional separada |
...
| 25-B / HU-077,086 | 1 por mensaje; destino solo-ciudad ya en un mensaje | Conservar pregunta m38; P8 solo contexto confiable | P1: 0 T/0 Q adicional; lookup repetido 0 T/Q | Sin cambio de pregunta propuesto |
...
Se conservan los IDs P1–P8 para trazabilidad; P1 queda invalidado fuera del ranking. El orden activo está en §1. Las prioridades usan beneficio relativo, certeza y riesgo; sin volumen real no existe ratio numérico defendible. Los IDs WF remiten al inventario live de §2. Pantallas: cuando aplica se usan los números de Matriz (p.ej. 17 para crear envío, 16/30/19 para seguimiento); las transformaciones internas no tienen pantalla asignada.

### P1 — Invalidado como ahorro; pregunta única actual verificada (WF-25-B; HU-077,086; pantalla 17)

WF-25-B `9doLr3FSewZofRoN`, versión `5bcc1c92-62a5-4876-a311-254c9dc150f7`, conserva **“¿A qué ciudad o población se envía?”**. Tras el prefijo de transportadora, pide un único dato concreto. Procesar Captura envía el texto original a `fn_procesar_captura_destino(p_telefono,p_texto)`; ¿Ciudad capturada? comprueba ENTREGA_REGISTRADA. Es coherente con OBS-005 corregida y bitácora final 2026-09-09.

El cuerpo final m38 vive en 34/06: trim de texto, ciudad limitada por recorte a 120 caracteres, zona NULL; dígitos reservados para re-selección de punto. `fn_entrega_captura_estado` devuelve paso DESTINO. La presencia residual de columnas destino_zona o nombres viejos de nodos no exige pedir zona.

**Conclusión:** claridad suficiente para una respuesta única; no se obtuvo evidencia de reenvíos causados por esa pregunta. **Ahorro incremental 0 T/0 Q.** Se retira la recomendación de pedir CIUDAD+ZONA y su estimación. No cambiar texto ni agregar pasos.

Como validación futura, medir correcciones atribuibles a esta pregunta y probar ciudad compuesta, texto vacío y re-selección numérica, sin inventar un formato obligatorio. Tablas: tbl_entrega_captura/tbl_envios. Es una comprobación de no regresión, no una intervención de ahorro.
...
**Tablas/fn:** tbl_notificaciones_envio (guarda visible en el JSON del corte v1), tbl_envios, tbl_whatsapp_eventos, tbl_logs_auditoria; fn_actualizar_estado_envio y **fn_registrar_guia**, firma y callback resueltos en 36/06 (§2.1). 25-A consulta pedido PAGADO y envía, sin dedup visible.
...
Un lote aquí significa **una raíz para varios grupos**, conservando una llamada aislada a 80 por destinatario. No implica cambiar WF-80 a batch. **Condición previa m40:** Build Message Payload de WF-13 usa `.first()` para id_comercio y nombre/precio/tiempo del resultado. Antes de ampliar/coalescer grupos, preservar correlación por ítem y sucursal; no reutilizar el primer resultado para todos. Validar una misma variante en dos sucursales con overrides distintos. El hecho de usar campos efectivos no prueba que un lote mixto esté correctamente emparejado.

### P5 — Reducir reenvíos por extracción/contexto (WF-21/22/23; HU-057–065,141,146; pantallas 24/06/07/25/08)
...
**Pasos futuros:** restaurar el contexto completo desde Parse Verification Result hacia 23; validar entrada antes de invocar hijos; tipar fallos de descarga/Storage/Gemini/parseo como técnicos, sin diagnóstico financiero; mantener D5 y revisión manual; confirmar que fn_* decide el resultado financiero y no Code por umbral; probar error técnico, reintento, confirmación concurrente y auditoría.

No proponer inline de Decide Verification como “transformación”: actualmente contiene una decisión de negocio que debe resolverse en fn_* conforme a Reglas 2/3/4. El ahorro de reenvíos es condicionado, no medido. **F1–F5 quedan excluidos**: P5 no reevalúa sus causas ni contabiliza su reparación. Los contratos SQL existentes son la referencia; no se piden como firmas desconocidas.
...
**Pasos futuros:** distinguir la opción de asistencia OCR de la decisión manual/automática; si el comercio desea modo sin OCR, trasladar captura durable de media fuera de la dependencia exclusiva de 22 y registrar RECIBIDO por el contrato autorizado, preservando revisión humana y D14. En automático, diseñar la secuencia futura a partir del contrato existente de fn_iniciar_verificacion en 06, sin convertir una mejora de secuencia en pedido de firma desconocida; no duplicar consumo de créditos ni bloquear una venta manual por saldo negativo.
...
03 tiene solo trigger + Canonical Message Normalizer + Execute 04. Es el candidato claro a inline del Code en el llamador, con contrato idéntico provider/message_id/phone/media/raw. **1 T menos por mensaje nuevo; 0 Q.** Conservar 04 como router. Confirmar primero que no haya callers no canónicos/OpenWA dependientes de 03. Su ahorro directo de cuota es 0; integra el quinto lugar por ahorro técnico y menor riesgo.
...
**WF-02/12/13/14/20/21/23/24/25-A/B/C; HU-124 y HU de cada rama:** estandarizar contrato de payload (id_comercio, phone, provider, type, text, media_url) y dueño del envío. Reutilizar formato en Code/Set dentro de los flujos pertinentes; no añadir un subworkflow meramente para construir texto bajo un objetivo de reducir T. El contexto de 23 queda en P5; la coordinación del aviso de 24 se remite a F1–F3 y no se vuelve a cuantificar.

**Restricción m40:** cualquier reutilización de resolución SKU debe conservar `(id_comercio, sku, id_sucursal)`; para valores de variante, `(id_variante,id_sucursal)`. Reutilizar el contexto dentro de la misma operación, nunca una caché global por SKU ni el precio de otra sucursal. Conservar los resultados efectivos de 10/13 y el snapshot de pedido de 20; no trasladar `fn_variante_efectiva` a Code/Set.
...
En **los 16 JSONs del corte live v1**, los tres campos solicitados están **ausentes** del objeto settings. Ausente significa **valor heredado/no explícito**, no false ni none. No se verificaron defaults efectivos de la instancia.
...
| 23 | Ausente | Ausente | Ausente | Igual; hasta validar propagación de contexto |
| 24 | Ausente | Ausente | Ausente | Conservar diagnóstico durante intervención F1–F3 separada |
| 25-A | Ausente | Ausente | Ausente | Evaluar false / none / all tras dedup/recuperación |
| 25-B | Ausente | Ausente | Ausente | Evaluar false / none / all; OBS-005 m38 ya resuelta |
...
- 21/22 confunden ciertos errores de extracción con no legibilidad; §5 P5. 23 puede emitir payloads incompletos. El caso de 24 pertenece a F1 y no se reaudita aquí. Son cascadas de trabajo o reenvío humano evitables, no evidencia de un bucle ilimitado.
...
8. **OBS-003/004/005:** puntos por sucursal, sin dirección libre/cobro de transporte, foto de guía/código y destino **solo ciudad/población en un mensaje (m38), sin zona ni formato obligatorio**; preservar saltos permitidos de estado. WF-25-A/B/C, HU-076,077,086,092–094.
...
10. **m40:** conservar overrides de nombre/precio/activo por sucursal, resolver de tres parámetros, permisos de catálogo y snapshot inmutable de pedido. WF-10/13/20; HU-037–045,056; tbl_variante_sucursal/tbl_pedido_detalles y fn_variante_efectiva. No reutilizar precio global ni el primer ítem para otras sucursales.
11. **WF-30 archivado/pg_cron:** no reactivar ni recrear expiración n8n, ni reducir garantías temporales o turnos para ahorrar ejecuciones. HU-049,052,053.

## 9. Pendiente de verificar — solo evidencia externa al vault

Las firmas m31/m34–38/m35/m36/m40 quedan **resueltas en §2.1**. Los demás contratos locales se consultan en SQL; no se trasladan al orquestador como documentación inexistente. No se hicieron lecturas SQL en la BD. Se distingue implementación documentada de configuración/telemetría efectivamente desplegada.

| Pendiente externo | Evidencia requerida / propósito |
|---|---|
| Panel Usage y medición | Plan/trial efectivo, raíces/status/retry frente a integrated; ventana representativa, retención y volumen de reenvíos. No usar count histórico como factura |
| Topología pg_cron→pg_net desplegada | Jobs habilitados y frecuencia efectiva, agrupación K, callbacks vacíos y política ACK/reintento. La ruta de fn_registrar_guia a 25-C está documentada; falta medir su entrega real, no su firma |
| Suscripción Meta/status y receptor | Estados realmente recibidos/necesarios, payloads mixtos, firma POST/autenticación y protección fuera de WF-02; factibilidad de filtrar antes de n8n sin perder trazabilidad |
| Defaults efectivos y operación | Valores heredados de guardado, retención, trazas de recuperación y cobertura efectiva de auditoría. Las declaraciones locales no certifican que un fallo de producción haya quedado registrado |
| Callers externos a los 16 y vigencia live | Otros consumidores de 03/OpenWA/error workflows; relectura previa a una futura intervención para detectar cambios posteriores al corte. 25-B/10/13 sí se reverificaron en v2 |

### Intervención funcional separada — F1–F5

Remisión al [paquete de intervención](PROMPT-INTERVENCION-FIXES-FUNCIONALES.md), que el orquestador ya verificó. **No ejecutado, no re-auditado ni cuantificado como ahorro en v2.**

| Paquete | Workflow / HU | Ámbito |
|---|---|---|
| F1–F3 | WF-24 `JU4QtP0vkAC7m3n1`; HU-076,078–081 | Destino de salida, IF y texto/coordinación de confirmación |
| F4 | WF-20 `1FYWXdVw2swlFYcg`; HU-042,056 | Ruta de imagen QR por comercio |
| F5 | WF-12 `n9VUH43N8i7s9Rn2`; HU-043,044 | Capacidad/duplicado y futura m41, a cargo del paquete separado |

No confundir m41 propuesta por F5 con m40 ya vigente. La estrategia de ahorro no prescribe ni contabiliza esos cinco fixes.

## 10. Secuencia futura y criterio de aceptación

1. **Mantener P1 cerrado:** la pregunta solo-ciudad cumple m38. No reintroducir zona ni formato obligatorio.
2. Medir raíces, hijos, status, repeticiones y errores por ventana; atribuir cada ahorro una sola vez. Coordinar con la intervención F1–F5, sin incorporar sus ahorros al presente informe.
3. Evaluar P2/P3/P4 con volumen real y garantías de recuperación; antes de agrupar WF-13, asegurar correspondencia por ítem/comercio/sucursal de los datos efectivos m40 y conservar WF-80 por destinatario.
4. Considerar P5/P6/P7/P8 y settings con sus límites. No pedir de nuevo firmas ya resueltas en SQL local.
5. Validación futura: SKU nuevo/reintento; misma variante en dos sucursales con precio/nombre/activo distintos; herencia NULL; snapshot previo que no cambia; SI/NO de ambos momentos; OCR manual/automático y fallo técnico; opt-out; callbacks repetidos; retiro/transporte con ciudad compuesta, vacío y re-selección; imagen de guía y recuperación tras fallo; auditoría. No se ejecutaron estas pruebas durante la auditoría.
6. En una implementación futura autorizada, actualizar Matriz/OBS. **En v2 se actualiza únicamente este informe; workflows y BD permanecen sin cambios.**

## Historial v1 — no vigente

> Copia íntegra anterior a esta revisión, incluido el addendum del orquestador. Se conserva únicamente como historial: no ejecutar sus propuestas ni usar sus pendientes/ahorros invalidados. La v2 precedente prevalece.

<details>
<summary>Ver informe v1 y addendum originales</summary>
